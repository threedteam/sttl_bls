...
class FeatureBlock(nn.Module):
    def __init__(
        ...
        self.conv5 = conv3x3(planes, planes, bias=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # se block
        identity = out
        seout = self.pool(out, self.conv5.bias)
        # seout = self.pool(out)
...
class GDBLS(nn.Module):
    def __init__(
        ...
        self.downsample1 = nn.AvgPool2d(kernel_size=4)
        self.conv2d1 = conv1x1(in_planes=filters[0], out_planes=filters[2])

        self.upsample2 = nn.UpsamplingBilinear2d(scale_factor=2)
        self.conv2d2 = conv1x1(in_planes=filters[1], out_planes=filters[0])
        self.flatten2 = torch.flatten
        self.downsample2 = nn.AvgPool2d(kernel_size=2)
        self.conv2d2 = conv1x1(in_planes=filters[1], out_planes=filters[2])
        ...
        self.fc = nn.Linear((filters[2] * 4 * 4), self.num_classes)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        out = self.fc(out)
        # p1 = torch.cat([p1, p2], 1)
        # p1 = torch.cat([p1, p3], 1)
        # out = self.fc(p1)
        return out
