...
class FeatureBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # seout = self.pool(out)
        # batchsize,planes,1,1 -> batchsize,planes
        seout = self.reshape1(seout, (self.batchs, self.planes))
        seout = self.fc1(seout)
        seout = self.fc2(seout)
        out = self.multiply(self.reshape2(
            seout, (self.batchs, self.planes, 1, 1,)), identity)
...
class GDBLS(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(
            (filters[0] * (self.inHW // 2) * (self.inHW // 2)), self.num_classes)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # [batchsize,32768]=batchsize*(128*16*16)
