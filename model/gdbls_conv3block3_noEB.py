...
class GDBLS(nn.Module):
    def __init__(
        ...
        outHW = [input_shape[1] // (2 ** (i + 1)) for i in range(3)]
        fcNodes = ([outHW[i] * outHW[i] * filters[i] for i in range(3)])
        # print(outHW)
        # print(fcNodes)
        self.fc = nn.Linear(sum(fcNodes), self.num_classes)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        p2 = self.flatten2(p2, start_dim=1)  # [batchsize,12288]=batchsize*(192*8*8)
        #
        # p3 = self.upsample3(p3)  # 4,4 -> 16,16, upsample factor = 4
        # p3 = self.conv2d3(p3)  # 1x1 conv
        p3 = self.flatten3(p3, start_dim=1)  # [batchsize,3072]=batchsize*(192*4*4)

        # for i in [p1, p2, p3]:
        #     print(f'shape={i.shape}\n')
        out = self.concat((p1, p2, p3), dim=1)
