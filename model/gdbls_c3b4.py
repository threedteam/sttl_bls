...
class GDBLS(nn.Module):
    def __init__(
        ...
        self.downsample1 = nn.AvgPool2d(kernel_size=8)
        self.conv2d1 = conv1x1(in_planes=filters[0], out_planes=filters[3])

        self.flatten2 = torch.flatten
        self.downsample2 = nn.AvgPool2d(kernel_size=4)
        self.conv2d2 = conv1x1(in_planes=filters[1], out_planes=filters[3])

        self.flatten3 = torch.flatten
        self.downsample3 = nn.AvgPool2d(kernel_size=2)
        self.conv2d3 = conv1x1(in_planes=filters[2], out_planes=filters[3])

        self.flatten4 = torch.flatten

        self.dropout = nn.Dropout(overall_dropout)
        self.fc = nn.Linear((filters[3] * 2 * 2), self.num_classes)
        ...
        self.encoder2 = Encoder(dim=filters[1], num_heads=num_heads[1], mlp_ratio=4, qkv_bias=True,
                                drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])

        self.encoder3 = Encoder(dim=filters[2], num_heads=num_heads[2], mlp_ratio=4, qkv_bias=True,
                                drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])

        self.encoder4 = Encoder(dim=filters[3], num_heads=num_heads[3], mlp_ratio=4, qkv_bias=True,
                                drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])

        self.encoder11 = Encoder(dim=filters[0], num_heads=num_heads[0], mlp_ratio=4, qkv_bias=True,
                                 drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])

        self.encoder22 = Encoder(dim=filters[1], num_heads=num_heads[1], mlp_ratio=4, qkv_bias=True,
                                 drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])

        self.encoder33 = Encoder(dim=filters[2], num_heads=num_heads[2], mlp_ratio=4, qkv_bias=True,
                                 drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])

        self.encoder44 = Encoder(dim=filters[3], num_heads=num_heads[3], mlp_ratio=4, qkv_bias=True,
                                 drop=dropout_rate[0], attn_drop=dropout_rate[0], drop_path=dropout_rate[0])
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        batchsize, _, H, W = p2.shape
        p2 = p2.flatten(2).transpose(1, 2)
        p2 = self.encoder2(p2, batchsize, H, W)
        p2 = p2.flatten(2).transpose(1, 2)
        p2 = self.encoder22(p2, batchsize, H, W)

        batchsize, _, H, W = p3.shape
        p3 = p3.flatten(2).transpose(1, 2)
        p3 = self.encoder3(p3, batchsize, H, W)
        p3 = p3.flatten(2).transpose(1, 2)
        p3 = self.encoder33(p3, batchsize, H, W)

        batchsize, _, H, W = p4.shape
        p4 = p4.flatten(2).transpose(1, 2)
        p4 = self.encoder4(p4, batchsize, H, W)
        p4 = p4.flatten(2).transpose(1, 2)
        p4 = self.encoder44(p4, batchsize, H, W)

        p1 = self.downsample1(p1)
        p1 = self.conv2d1(p1)
        p1 = self.flatten1(p1, start_dim=1)

        p2 = self.downsample2(p2)
        p2 = self.conv2d2(p2)
        p2 = self.flatten2(p2, start_dim=1)

        p3 = self.downsample3(p3)
        p3 = self.conv2d3(p3)
        p3 = self.flatten3(p3, start_dim=1)

        p4 = self.flatten4(p4, start_dim=1)

        out = self.dropout(p1 + p2 + p3 + p4)
        out = self.fc(out)
        return out
