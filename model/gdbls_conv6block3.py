...
class FeatureBlock(nn.Module):
    def __init__(
        ...
        self.conv5 = conv3x3(planes // 2, planes // 2)
        self.bn5 = nn.BatchNorm2d(planes // 2)
        self.relu5 = nn.ReLU(inplace=True)
        self.dropout5 = nn.Dropout(dropout_rate)

        if islast:
            self.conv6 = conv5x5(planes // 2, planes, bias=True)
        else:
            self.conv6 = conv3x3(planes // 2, planes, bias=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.relu4(out)
        out = self.bn4(out)
        out = self.dropout4(out)  # batchsize,planes // 2,32,32

        out = self.conv5(out)
        out = self.relu5(out)
        out = self.bn5(out)
        out = self.dropout5(out)  # batchsize,planes // 2,32,32

        out = self.conv6(out)
        out = self.relu6(out)  # batchsize,planes,32,32
        # out = self.bn3(out)
