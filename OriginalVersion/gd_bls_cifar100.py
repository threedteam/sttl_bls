...
def getTrainValTestDatas():
    # 从keras中载入数据集cifar100
    (X_train, Y_train), (x_test, y_test) = cifar100.load_data()  # X_train(50000,32,32,3),Y_train(50000,1)
...
def create_cfbbls_2blocks_2convs():
    ...
    d1 = Dropout(0.6)(merged)

    model_y = Dense(100, activation='softmax', kernel_initializer=initializers.lecun_normal(seed=None),
...
if __name__ == '__main__':
    ...
    std = np.array([0.2023, 0.243, 0.262])
    x_train = (x_train.astype('float') / 255.0 - mean) / std
    x_val = (x_val.astype('float') / 255.0 - mean) / std
    x_test = (x_test.astype('float') / 255.0 - mean) / std
